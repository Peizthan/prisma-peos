...
// Mirrors the exact field labels the live Google Form response sheet uses.
...
interface FormData {
  ...
  email: string;
  serviceType: (typeof SERVICE_OPTIONS)[number] | '';
  packageName: (typeof PACKAGE_OPTIONS)[number] | '';
...
function validate(data: FormData): FieldErrors {
  ...
  if (!data.serviceType) errors.serviceType = 'Seleccioná una opción';
  if (!data.packageName) errors.packageName = 'Seleccioná un paquete';
  if (!data.delivery) errors.delivery = 'Seleccioná una opción';
  if (!data.pixieset) errors.pixieset = 'Seleccioná una opción';
  return errors;
}
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-6">
      ...
      <Section title="Datos del atleta">
        <TextField
          label="Nombre y apellido del atleta"
          value={form.athleteFullName}
          onChange={(v) => set('athleteFullName', v)}
          error={errors.athleteFullName}
        ...
        <TextField
          ...
          onChange={(v) => set('guardianFullName', v)}
        ...
        <TextField
          ...
          onChange={(v) => set('phoneWhatsapp', v)}
        ...
        />
        <TextField
          label="Academia / grupo / club"
          value={form.academyGroupClub}
          onChange={(v) => set('academyGroupClub', v)}
      ...
      <Section title="Servicio">
        <RadioGroup
          label="¿Qué tipo de servicio querés?"
          options={SERVICE_OPTIONS}
          value={form.serviceType}
          onChange={(v) => set('serviceType', v as FormData['serviceType'])}
        ...
        <RadioGroup
          ...
          onChange={(v) => set('packageName', v as FormData['packageName'])}
      ...
      <Section title="Entrega y extras">
        <RadioGroup
          ...
          value={form.delivery}
          onChange={(v) => set('delivery', v as FormData['delivery'])}
          error={errors.delivery}
          required
        />
        <RadioGroup
          label="¿Te gustaría elegir tus fotos con Pixieset?"
          options={['Sí', 'No'] as const}
          value={form.pixieset}
          onChange={(v) => set('pixieset', v as FormData['pixieset'])}
          error={errors.pixieset}
          required
        />
      ...
      <button
        ...
        className="w-full rounded-lg bg-brand-accent py-4 font-display text-xl tracking-widest text-brand-bg
                   transition-colors hover:bg-brand-accent-dark disabled:opacity-60"
      >
      ...
      <div className="flex flex-col gap-2">
        {options.map((opt) => {
          ...
          return (
            <label
              key={opt}
              className={`radio-card${selected ? ' selected' : ''}${error ? ' error' : ''}`}
            >
              <span
